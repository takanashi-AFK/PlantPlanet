...
void UIButton::DrawData()
{
	...
	if (ImGui::TreeNode("imageFilePath_")) {
		...
		{
			imageFilePath_ = {};
			selectImageFilePath_ = {};
			defaultImageFilePath_ = {};
			imageHandle_ = -1;
			selectImageHandle_ = -1;
			defaultImageHandle_ = -1;
			isChangeImage_ = false;
		}
...
bool UIButton::OnClick()
{
	...
	return IsMouseOver(mousePos) && this->isVisible_ && (Input::IsMouseButtonDown(0) || Input::IsPadButtonDown(XINPUT_GAMEPAD_A));
}
